<template>
  <v-container class="approvals" v-if="job">
    ...
    <p v-if="!job.shifts || !job.shifts.length" class="text-body-2 text-center mt-3">
...
export default {
  ...
  methods: {
    ...
    organizationManagerName(organizationManagerId) {
      ...
      const m = this.job.managers.organization_managers.find((m) => m.manager_id == organizationManagerId);
      return m ? `${m.first_name} ${m.last_name}` : '';
    },
    employeeManagerName(employeeManagerId) {
      if (!this.job.managers) return '';
      const m = this.job.managers.employee_managers.find((m) => m.id == employeeManagerId);
      return m ? `${m.first_name} ${m.last_name}` : '';
    },
    addShift() {
      return;
