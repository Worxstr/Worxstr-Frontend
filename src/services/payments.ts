...
export async function addFundingSource(
  ...
  commit('SHOW_SNACKBAR', {
    text:
      'Deposit transfer initiated. Check your online banking in 2-3 business days to verify this deposit.'
  })
  return data
}

export async function addPlaidFundingSource(
  { commit }: any,
  {
    accountName,
    publicToken,
...
export async function verifyFundingSource(
  { commit }: any,
  {
    fundingSourceUrl,
    amount1,
...
export async function removeFundingSource(
  { commit }: any,
  fundingSourceLocation: string
) {
  const { data } = await api({
    method: 'DELETE',
    url: 'payments/accounts',
...
export async function addToBalance(
  { commit }: any,
  transfer: { amount: number; location: string }
) {
  const { data } = await api({
    method: 'POST',
  ...
  showToast(
    { commit },
    { text: 'Hang tight, your transfer is being processed.' }
  )
  return data
}

export async function removeFromBalance(
  { commit }: any,
  transfer: { amount: number; location: string }
) {
  const { data } = await api({
